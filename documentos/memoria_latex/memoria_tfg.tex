...
\begin{itemize}
    ...
    (p. ej., dado un conjunto de características de un paciente, predecir qué enfermedad tiene: salida categórica) y la 
    ...
    que relacionan la predicción del modelo con el valor real (p. ej. error cuadrático en regresión o entropía cruzada en clasificación).
...
En el ámbito médico se utiliza comúnmente el \textbf{aprendizaje supervisado}. El proceso típico de esta tarea consiste en entrenar
...
La Figura~\ref{fig:gd_flow} muestra esquemáticamente el flujo de este proceso, donde la posición inicial tiene el error máximo y en cada
...
donde cada neurona está conectada solo a una región local de la capa anterior (campo receptivo) y todas los neuronas de una capa comparten conjuntos de pesos (filtros)
...
(p. ej. clasificando imágenes por el fondo en lugar del objeto de interés). 
