...
El \textbf{aprendizaje automático en imágenes médicas} (\emph{medical imaging}) se apoya en los mismos principios que en visión por computador general, pero presenta particularidades técnicas y clínicas que condicionan tanto el pipeline de datos como el diseño, la validación y el despliegue de modelos. A diferencia de las imágenes naturales, las imágenes clínicas se adquieren bajo protocolos estandarizados, se codifican con metadatos (p.,ej., en formato DICOM) y derivan de modalidades con física de adquisición distinta (radiografía, TC, RM, ecografía o microscopía). Además, los objetivos habituales van más allá de la simple \emph{clasificación}: incluyen \textbf{detección}, \textbf{segmentación}, \textbf{estimación ordinal de severidad}, \textbf{calibración de riesgo} o \textbf{pronóstico}. Esta diversidad de tareas, junto con la escasez de datos anotados por expertos y la necesidad de interpretabilidad, explica por qué el área ha adoptado con rapidez las técnicas modernas de \emph{deep learning} \cite{Mitchell1997,Bishop2006,Goodfellow2016}.

\medskip
\noindent\textbf{Pipeline típico de trabajo.}; Un flujo estándar comprende: (1) \emph{ingesta y curación} (lectura DICOM/PNG, verificación de metadatos); (2) \emph{preprocesado} específico por modalidad (p.,ej., normalización de intensidades, aumento de contraste...); (3) \emph{homogeneización geométrica} (recortes centrados, reescalado, estandarización de orientación); (4) \emph{aumento de datos} con transformaciones geométricas e intensidad \emph{domain-aware}; (5) entrenamiento con división \emph{train/val/test} estratificada; (6) \emph{validación interna} y, a ser posible,\textbf{validación externa} (en centros distintos o cohortes temporales) para medir la robustez; (7) análisis de \emph{calibración}, \emph{curvas ROC/PR} y \emph{sensibilidad/especificidad}; (8) \textbf{interpretabilidad} (p.,ej., \emph{Grad-CAM}) y chequeo clínico; (9) empaquetado e integración.

\medskip
\noindent\textbf{Modelos: de CNN a arquitecturas modernas.}; El salto de calidad en visión vino dado por las \textbf{CNN profundas}, introducidas con \emph{AlexNet} en ImageNet \cite{Krizhevsky2012} y consolidadas con \emph{ResNet} y las conexiones residuales \cite{He2016}. En clínica, las CNN resultan adecuadas por su capacidad para extraer \emph{patrones jerárquicos} y por la disponibilidad de pesos preentrenados en dominios masivos (\emph{transfer learning}). En problemas de \textbf{gradación ordinal} (p.,ej., escalas 0-4) se han propuesto pérdidas específicas que penalizan según la distancia del error, mejorando frente a entropía cruzada cuando las clases forman una \emph{ordenación} natural, como ilustra Chen et al. en rodilla \cite{chen2019fully}. Más recientemente, arquitecturas basadas en mecanismos de \emph{atención} (Transformers de visión) han ganado tracción; en entornos clínicos, a menudo se emplean de forma híbrida o mediante técnicas de preentrenamiento y ajuste fino, especialmente cuando los datos anotados son limitados.

\medskip
\noindent\textbf{Evidencia de rendimiento y adopción clínica.}; Revisiones sistemáticas y meta-análisis han comparado el rendimiento de DL con profesionales sanitarios en distintas tareas de imagen; por ejemplo, \cite{liu_meta_analysis} reporta que, en varios escenarios, los modelos alcanzan desempeños comparables en
sensibilidad/especificidad, si bien subraya la \textbf{importancia de la validación externa} y la \textbf{calidad metodológica} de los estudios. La traducción a productos comerciales certificados está en marcha (p.,ej., suites radiológicas con módulos específicos de órgano) \cite{quibim_qp_prostate}, pero exige procesos de aseguramiento, trazabilidad y evaluación continua en \emph{mundo real}.

\medskip
\noindent\textbf{Caso de estudio frecuente: rodilla y artrosis.}; La articulación de la rodilla es un ejemplo clásico por la prevalencia de la osteoartritis y la existencia de \textbf{cohortes longitudinales} de acceso público. En particular, la \emph{Osteoarthritis Initiative} (OAI) ha recogido imágenes y metadatos durante más de una década \cite{niamsOAI}. Sobre esta base, hay colecciones curadas de las radiografías de rodilla con escalas \emph{Kellgren-Lawrence} (KL) \cite{chen2018knee}, y trabajos que combinan \emph{detección de articulación} con clasificación/graduación (incluyendo pérdidas ordinales) \cite{chen2019fully}. Estos recursos han facilitado la comparación de arquitecturas (VGG/ResNet/EfficientNet) y de formulaciones de tarea (multiclase vs. ordinal) en un entorno reproducible.
...
\medskip
\noindent\textbf{Particularidades del preprocesado en radiografías.}; En radiografía simple, el contraste depende de factores como kilovoltaje, filtrado y grosor del paciente. Es habitual normalizar intensidades, ajustes de rango dinámico y, para modelos centrados en una articulación concreta, \textbf{recortar la ROI} con detectores o heurísticas anatómicas. Estas operaciones reducen la variación inter-centro y ayudan a que el modelo se centre en \emph{pistas radiográficas} relevantes (osteofitos, esclerosis subcondral, pinzamiento del espacio articular, etc.).

\medskip
\noindent\textbf{Métricas y análisis estadístico.}; En \emph{clasificación} multiclase se reportan \emph{accuracy}, \emph{F1 macro} y matrices de confusión por clase; en \emph{binaria} es común AUC-ROC/PR junto a sensibilidad/especificidad para umbrales clínicos. En formulaciones \textbf{ordinales/regresivas} se incluye el MAE y estadísticos de calibración (\emph{reliability diagrams}). La \textbf{validación externa} en centros distintos y la evaluación por subgrupos (edad, sexo, dispositivo) son claves para estimar \emph{generalización} y \emph{equidad} \cite{liu_meta_analysis}. También resulta recomendable contrastar el acuerdo con expertos mediante coeficientes tipo \emph{Cohen/Fleiss kappa} cuando proceda.

% [FIGURA SUGERIDA]
% \begin{figure}[ht]
%   \centering
%   \includegraphics[width=0.6\textwidth]{roc_placeholder.pdf}
%   \caption{Curva ROC y selección de umbral según preferencia clínica (balance sensibilidad/especificidad).}
%   \label{fig:roc}
% \end{figure}

\medskip
\noindent\textbf{Interpretabilidad y verificación clínica.}: La \emph{explicabilidad} busca comprobar que el modelo basa sus decisiones en regiones anatómicas coherentes. Mapas de activación como \emph{Grad-CAM} permiten superponer \emph{heatmaps} sobre la imagen original, ayudando a detectar atajos espurios (p.,ej., artefactos o marcas de la placa) y a construir confianza clínica. Aunque la explicabilidad no sustituye a la validación es una prueba extra que ayuda a determinar la capacidad real del modelo.

% [FIGURA SUGERIDA]
% \begin{figure}[ht]
%   \centering
%   % Imagen original + heatmap Grad-CAM como overlay
%   \includegraphics[width=0.95\textwidth]{gradcam_placeholder.png}
%   \caption{Ejemplo esquemático de mapa de activación (Grad-CAM) resaltando la región que apoya la predicción del modelo.}
%   \label{fig:gradcam}
% \end{figure}

\medskip
\noindent\textbf{Retos recurrentes.}; Entre los desafíos más citados se encuentran: (i) \textbf{desbalance de clases} (p.,ej., escasez de grados severos), que requiere \emph{estrategias de muestreo}, ponderación de pérdidas o síntesis \emph{data augmentation}; (ii) \textbf{cambio de dominio} entre centros, equipos y protocolo (\emph{domain shift}); (iii) \textbf{ruido de etiqueta} e \textbf{incertidumbre} interobservador; (iv) \textbf{tamaño muestral} limitado; (v) \textbf{calibración} deficiente pese a buena AUC; (vi) \textbf{reproducibilidad} (documentación de versiones, semillas, splits) y \textbf{privacidad}; (vii) requisitos regulatorios y \emph{monitorización post-despliegue}. La literatura recomienda predefinir protocolos de validación, fijar semillas, conservar \emph{splits} oficiales cuando existan y acompañar resultados de \emph{intervalos de confianza}.

\medskip
\noindent\textbf{Buenas prácticas resumidas.}; A modo de checklist técnico:
\begin{itemize}
    \item Definir claramente tarea, métrica primaria y \emph{endpoints} clínicos; separar \emph{train/val/test} sin contaminación temporal o por paciente.
    \item Preprocesado específico por modalidad y normalización consistente; documentar resoluciones y escalados.
    \item Manejo del desbalance (ponderación de pérdidas, \emph{oversampling} minoritario).
    \item Evaluación con métricas apropiadas (AUC/PR en clases desbalanceadas; MAE en ordinal/regresión) y análisis por subgrupos.
    \item Validación externa cuando sea posible; análisis de calibración y curvas de decisión.
    \item Interpretabilidad (p.,ej., \emph{Grad-CAM}) revisada por expertos.
    \item Fijar semillas, registrar versiones y publicar detalles suficientes para reproducibilidad.
\end{itemize}

\medskip
\noindent\textbf{Síntesis.}; El uso de aprendizaje profundo en imagen médica ha pasado de estudios en laboratorios a validaciones multicéntricas y primeros productos comerciales. Las CNN profundas y las pérdidas adaptadas a ordinalidad han demostrado utilidad en problemas como la
artrosis de rodilla (cohortes OAI), mientras que la evidencia comparativa con  profesionales sugiere viabilidad siempre que existan pipelines de validación y control de calidad robustos. De cara a su adopción, los aspectos de calibración, generalización y explicabilidad
son tan relevantes como el valor central de la métrica.
