...
Dado el reducido tamaño del conjunto felino y la subjetividad de las clases más leves, se ha llevado a cabo una selección cuidadosa de las tareas más razonables. En particular, se han descartado aquellas con escasa diferencia visual o clínica. La elección de estás taraes no han sido al tum tum, si no que gracias a Bea, la veterinaria responsable de darme los datos hemos seleccionado las mejores tareas


\section{Experimentos realizados}

\subsection{Entrenamiento desde cero}
Como línea base, se ha entrenado un modelo CNN desde cero utilizando únicamente las imágenes de gatos. El modelo ha mostrado una fuerte tendencia al sobreajuste, lo que confirma la necesidad de recurrir a técnicas de transferencia de conocimiento.

\subsection{Fine-tuning desde modelos preentrenados}

\subsubsection{Modelos preentrenados en OAI}
Se ha reutilizado el modelo entrenado en humanos y se ha ajustado únicamente la cabeza de clasificación y las capas superiores. Esta estrategia ha resultado especialmente efectiva en tareas con mayor diferencia visual.

\subsubsection{Modelos preentrenados en ImageNet}
Se ha probado también el fine-tuning desde arquitecturas preentrenadas en ImageNet (por ejemplo, EfficientNetB0). Si bien el rendimiento ha sido aceptable, no ha superado al modelo transferido desde OAI en la mayoría de tareas.

\section{Aplicación de técnicas de aumento de datos}
Se ha implementado \textit{data augmentation} en tiempo real para mitigar el riesgo de sobreajuste. Las transformaciones incluyen:

\begin{itemize}
    \item Rotación aleatoria ($\pm 10^\circ$)
    \item Inversión horizontal
    \item Ajustes de brillo y contraste
\end{itemize}

Esta técnica ha demostrado ser crucial para mejorar el rendimiento en tareas multiclase y binarizadas con clases intermedias.
